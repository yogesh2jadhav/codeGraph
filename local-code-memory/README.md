...
| 13 | Advisor task modes — `--mode implement_feature\|explain_code\|find_fix\|debug\|add_logging\|refactor\|impact_analysis` | ✅ done |
...
`--mode` picks the advisor prompt (`implement_feature` default, `explain_code`,
`find_fix`, `debug`, `add_logging`, `refactor`, `impact_analysis`); `--patch`
also asks for a `patch.diff` and runs `git apply --check` on it — it is never
applied for you.

`--mode explain_code` is for "what does this method actually do" — a plain
prose, line-by-line walkthrough (parameters, branches, calls made, tables/
config touched, what's returned) with **no proposed changes**, unlike every
other mode:

```bash
../.venv/bin/python -m code_memory --project /path \
  context "explain what CasingService.getClaimDataForSparkJRE does end to end" \
  --ask --mode explain_code
```
